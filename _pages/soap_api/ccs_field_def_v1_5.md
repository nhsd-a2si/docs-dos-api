...
| attributes	|  Array |       |
|  dataType, name, description, value  | | Each service may have 0 – many attributes, and each will be included here. An attribute is identified by a name and will be returned with a value of an identified data type ([further details](https://developer.nhs.uk/apis/dos-api/ccs_field_def_v1_5_service_attribute.html)). It is possible for a service to have multiple instances of the same attribute with different values. Intially this will return no data |
